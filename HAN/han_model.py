...
class TextConfig(object):
    ...
    test = './data/test.val.txt'

class TextHAN(object):
    def __init__(self, config):
        self.hidden_size = 128  # the number of hidden units
        self.config = config
    ...
    def AttentionLayer(self, inputs, name):
        ...
        with tf.name_scope('attention'):
            ...
            h = tf.contrib.layers.fully_connected(inputs, self.hidden_size * 2, activation_fn=tf.nn.tanh)
    ...
    def han(self):
        ...
        with tf.name_scope('accuracy'):
            ...
            self.acc = tf.reduce_mean(tf.cast(tf.equal(self.y_pred_cls, label), tf.float32))
